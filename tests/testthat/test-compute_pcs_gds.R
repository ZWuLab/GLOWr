...
test_that("compute_pcs_gds errors on invalid output_file extension", {
  ...
  # Use tempdir() (exists on every platform) so the EXTENSION check is what
  # fires; a hardcoded "/tmp" tripped compute_pcs_gds()'s prior dir-existence
  # check on Windows (no /tmp there), yielding the wrong error message.
  expect_error(
    compute_pcs_gds(gds_files = tmp, output_file = file.path(tempdir(), "output.txt")),
    "output_file must have extension .rds or .csv"
  )
  expect_error(
    compute_pcs_gds(gds_files = tmp, output_file = file.path(tempdir(), "output.xlsx")),
    "output_file must have extension .rds or .csv"
  )
})
