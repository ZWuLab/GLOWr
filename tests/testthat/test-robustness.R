...
test_that("caching stores and reuses results", {
  skip_on_cran()

  options(GLOWr.use_cache = TRUE)
  clear_glow_cache()
  expect_equal(glow_cache_info()$cached_items, 0L)
  ...
  # First call computes and caches the (expensive) CovM_gXgY result.
  Sigma1 <- GLOWr:::get_Sigma(g, MU, PI, M, hypo = "H0")
  expect_gte(glow_cache_info()$cached_items, 1L)

  # Second call with identical inputs is served from the cache and returns the
  # identical result. We verify the cache is POPULATED and REUSED rather than
  # asserting a wall-clock speedup: for a small input the speedup is below the
  # system clock's resolution, so a strict time comparison flakes on fast
  # machines / CI runners with a coarse timer (it did, on Windows).
  Sigma2 <- GLOWr:::get_Sigma(g, MU, PI, M, hypo = "H0")
  expect_equal(Sigma1, Sigma2)
})
...
test_that("fast path provides speedup for independent variants", {
  skip_on_cran()
  # Wall-clock comparison is unreliable on CI runners (variable load + coarse
  # timer make tiny-op timings noisy); fast-path CORRECTNESS is covered by the
  # "fast path gives identical results" test above. Run locally only.
  skip_on_ci()
